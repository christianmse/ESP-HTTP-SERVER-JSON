...
static esp_err_t temperature_f_data_get_handler(httpd_req_t *req)
{
    httpd_resp_set_type(req, "application/json");
    cJSON *root = cJSON_CreateObject();
    float temperature_fahrenheit = ((float) (esp_random() % 20) * 9/5) + 32.0f;
    cJSON_AddNumberToObject(root, "raw", temperature_fahrenheit);
    const char *sys_info = cJSON_Print(root);
    httpd_resp_sendstr(req, sys_info);
    free((void *)sys_info);
    cJSON_Delete(root);
...
esp_err_t start_rest_server(const char *base_path)
{
    ...
    /* URI handler for getting temperature data in Fahrenheit */
    httpd_uri_t temperature_f_data_get_uri = {
        .uri = "/api/v1/temp_f/raw",
        .method = HTTP_GET,
